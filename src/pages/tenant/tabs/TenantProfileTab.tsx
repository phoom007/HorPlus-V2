...
import { User, ChevronLeft, Wrench, AlertCircle, LogOut, Phone, Shield, Edit3, Plus, Trash2, Car, Bike, PawPrint } from 'lucide-react';
        ...
        <div className="bg-white p-4 border border-slate-100 rounded-3xl space-y-4 shadow-2xs">
          <div className="flex justify-between items-center">
            <div className="flex gap-3.5 items-center">
              <div className="w-12 h-12 rounded-2xl bg-indigo-50 text-indigo-700 flex items-center justify-center font-black text-base border border-indigo-100/60 shadow-2xs">
                {(localTenant?.name || 'ผ').charAt(0)}
              </div>
              <div>
                <h4 className="font-black text-slate-800 text-sm">คุณ{localTenant?.name || 'ผู้เช่า'}</h4>
            ...
            >
              ...
              <span>{isEditingInfo ? 'ปิดการแก้ไข' : 'แก้ไขข้อมูล'}</span>
            </button>
          </div>
          ...
          <div className="border-t border-slate-100 pt-3 text-[10px] space-y-2 text-slate-600 leading-normal">
            ...
            <p>
              ...
              <span className="font-bold text-indigo-700">
                ห้อง {roomNumber || '-'} {buildingName ? `∙ ${buildingName}` : ''}
              </span>
            </p>
            <p>
          ...
          {/* Inline Multi-Vehicle & Multi-Pet Edit Form */}
          {isEditingInfo && (
            <form
              onSubmit={handleSubmit}
              className="border-t border-slate-100 pt-3 space-y-3 animate-in fade-in duration-150"
            >
              <h5 className="font-extrabold text-slate-800 text-xs">
                แก้ไขข้อมูลยานพาหนะและสัตว์เลี้ยง
              </h5>
