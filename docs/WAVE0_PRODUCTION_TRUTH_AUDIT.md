...
| `/tenant/profile` | Sample profile data, synthetic PDF defaults (18, 7, 25, 5, 1, 0, 2026-01-01) | Purged all synthetic defaults, primary authority bound to immutable ContractSnapshot first, formatted rates cleanly (0 -> "0.00", missing -> "ไม่ระบุ"), internet fee formatted cleanly without forced zero conversion, verified immutability when dormitory billing settings change. | ContractSnapshot & Tenant Profile API | LINE account linking | Wave 1 | `TRUTH-SAFE` |

## Verification Gates
1. **Architectural Gate (`src/tests/productionTruthBoundary.test.ts`)**: Asserts 0 `mockData` imports in runtime paths, `getDataMode() === 'api'`, 0 forbidden external QR URLs, 0 `meters_state_*` / numeric rate fallbacks in `meters.tsx`, 0 reset demo data in `settings.tsx`, 0 `demoHasRoom` or `A-101` in `tenant.tsx`, 0 billingDay/dueDay fallback 1, 0 `internetFee || '0.00'`, 0 synthetic date fallback `2026-01-01T00:00:00.000Z`, and 0 hardcoded 18/7/101/25/5 defaults in server contract PDF route.
...
3. **Contract PDF Unit Suite (`server/tests/contract-pdf-truth.test.ts`)**: Validates snapshot primary authority, distinctive value assertions (Z909, 4321.00, 8765.00, 21.25, 8.75, 123.45, 246.80, 15, 5), zero value preservation ("0.00"), missing value fallback ("ไม่ระบุ"), and strict PDF immutability against dormitory billing setting mutations.
