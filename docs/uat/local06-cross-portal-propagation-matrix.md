...
| **PROP-004** | OWNER / TECH | Enter Meter Readings (`/owner/meters`) | TENANT | Itemized utility units & draft bill generated; Tenant invoice reflects water/electric charges | `meter_readings` inserted, `bills` updated, `bill_items` inserted | Bill issued notification | Meter readings intact on F5 | PASS |
...
| **PROP-010** | TENANT | Manage Co-Occupants (Add / Delete) | OWNER / MANAGER | Co-occupant list on Owner Tenant Profile updates immediately | `tenant_co_occupants` inserted / deleted | None | Co-occupants intact on F5 | PASS |
...
3. **Auditability**: Critical business events generate structured audit records in database domain tables.
