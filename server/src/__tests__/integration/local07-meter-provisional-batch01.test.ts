...
describe('LOCAL-07 Batch 01 — Meter Core & Provisional Rental Terms Test Suite', () => {
  ...
  // --------------------------------------------------------------------------
  // TEST 30: Daily Half-Open Boundary & Attribution
  // --------------------------------------------------------------------------
  it('30. Daily Half-Open Boundary: Stay ending exactly on cycle start boundary is excluded from that cycle', async () => {
    const dId = crypto.randomUUID();
    const dorm = await prisma.dormitory.create({
      data: {
        id: dId,
        name: 'Dorm Boundary Test',
        billingSettings: {
          create: {
            waterBillingType: 'per_unit',
            waterRate: toDecimal('18.00'),
    ...
    const bld = await prisma.building.create({
      data: { dormitoryId: dId, name: 'Bld 1' },
    });
    ...
    const boundaryRoom = await prisma.room.create({
      data: {
        dormitoryId: dId,
        buildingId: bld.id,
        roomNumber: 'B-BOUND-1',
        normalizedRoomNumber: 'B-BOUND-1',
        roomType: 'standard',
        monthlyRent: 3000,
        status: 'vacant',
      },
    ...
    try {
      // Stay A: Aug 31 12:00 -> Sep 01 00:00 Bangkok
      const stayA = await prisma.dailyStay.create({
        data: {
          dormitoryId: dId,
          roomId: boundaryRoom.id,
          applicantFullName: 'ผู้พักสิ้นสุดเขตแดน',
          startDate: new Date('2026-08-31T00:00:00.000Z'),
          endDate: new Date('2026-08-31T00:00:00.000Z'),
          checkInAt: new Date('2026-08-31T05:00:00.000Z'), // 12:00 Bangkok
          checkOutAt: new Date('2026-08-31T17:00:00.000Z'), // 2026-09-01 00:00 Bangkok
          inclusiveDayCount: 1,
          dailyRateAmount: 500,
          totalRentAmount: 500,
          depositAmount: 0,
      ...
      // Aug cycle preview: should include Stay A
      const augPreview = await meterService.getMeterBillingPreviewContext(dId, augCycle.id);
      const augRoom = augPreview.rooms.find(r => r.roomId === boundaryRoom.id);
      expect(augRoom?.historicalDailyCount).toBe(1);

      // Sep cycle preview: must exclude Stay A because checkOutAt == Sep 01 00:00 Bangkok
      const sepPreview = await meterService.getMeterBillingPreviewContext(dId, sepCycle.id);
      const sepRoom = sepPreview.rooms.find(r => r.roomId === boundaryRoom.id);
      expect(sepRoom?.historicalDailyCount).toBe(0);

      await prisma.dailyStay.deleteMany({ where: { id: stayA.id } });
    } finally {
      await prisma.billingRateSnapshot.deleteMany({ where: { dormitoryId: dId } });
      await prisma.billingCycle.deleteMany({ where: { dormitoryId: dId } });
  ...
  // --------------------------------------------------------------------------
  // TEST 31: Daily RENT-Specific Unpaid Authority & Deposit Independence
  // --------------------------------------------------------------------------
  it('31. Daily RENT Authority: Deposit paid does not clear isDailyUnpaid when RENT is unpaid', async () => {
    const dId = crypto.randomUUID();
    await prisma.dormitory.create({
      data: {
        id: dId,
        name: 'Dorm Rent Auth Test',
        billingSettings: {
          create: {
            waterBillingType: 'per_unit',
            waterRate: toDecimal('18.00'),
    ...
    const rentAuthRoom = await prisma.room.create({
      data: {
        dormitoryId: dId,
        buildingId: bld.id,
        roomNumber: 'B-RENT-1',
        normalizedRoomNumber: 'B-RENT-1',
        roomType: 'standard',
        monthlyRent: 3000,
        status: 'vacant',
      },
    ...
    try {
      const stay = await prisma.dailyStay.create({
        data: {
          dormitoryId: dId,
          roomId: rentAuthRoom.id,
          applicantFullName: 'สมปอง ค่าประกันจ่ายแล้ว',
          startDate: new Date('2026-08-10T00:00:00.000Z'),
          endDate: new Date('2026-08-12T00:00:00.000Z'),
          checkInAt: new Date('2026-08-10T07:00:00.000Z'),
          checkOutAt: new Date('2026-08-12T05:00:00.000Z'),
          inclusiveDayCount: 2,
          dailyRateAmount: 500,
          totalRentAmount: 1000,
          depositAmount: 500,
          status: 'CHECKED_OUT',
        },
      });

      const inv = await prisma.dailyStayInvoice.create({
        data: {
          dormitoryId: dId,
          dailyStayId: stay.id,
          invoiceNumber: `INV-D-2026-08-9991`,
          totalAgreedAmount: toDecimal('1500.00'),
          totalRentAmount: toDecimal('1000.00'),
          depositAmount: toDecimal('500.00'),
          status: 'ISSUED',
        },
      });

      // DEPOSIT item is SETTLED/PAID, but RENT item is UNPAID
      await prisma.dailyStayInvoiceItem.create({
        data: {
          invoiceId: inv.id,
          itemType: 'DEPOSIT',
          description: 'ค่าประกันความเสียหาย',
          amount: 500,
          status: 'SETTLED',
      ...
      await prisma.dailyStayInvoiceItem.create({
        data: {
          invoiceId: inv.id,
          itemType: 'RENT',
          description: 'ค่าเช่ารายวัน 2 วัน',
          amount: 1000,
          status: 'PENDING_PAYMENT',
        },
      });

      // August preview: isDailyUnpaid must remain TRUE because RENT is unpaid
      const preview1 = await meterService.getMeterBillingPreviewContext(dId, augCycle.id);
      const room1 = preview1.rooms.find(r => r.roomId === rentAuthRoom.id);
      expect(room1?.isDailyUnpaid).toBe(true);
      expect(room1?.tenantName).toBe('สมปอง ค่าประกันจ่ายแล้ว');

      // Now settle RENT item with legacy paidAt: null
      await prisma.dailyStayInvoiceItem.updateMany({
        where: { invoiceId: inv.id, itemType: 'RENT' },
        data: { status: 'SETTLED', paidAt: null },
      });

      // August preview: isDailyUnpaid becomes FALSE
      const preview2 = await meterService.getMeterBillingPreviewContext(dId, augCycle.id);
      const room2 = preview2.rooms.find(r => r.roomId === rentAuthRoom.id);
      expect(room2?.isDailyUnpaid).toBe(false);

      await prisma.dailyStayInvoiceItem.deleteMany({ where: { invoiceId: inv.id } });
      await prisma.dailyStayInvoice.deleteMany({ where: { id: inv.id } });
      await prisma.dailyStay.deleteMany({ where: { id: stay.id } });
    } finally {
      await prisma.billingRateSnapshot.deleteMany({ where: { dormitoryId: dId } });
      await prisma.billingCycle.deleteMany({ where: { dormitoryId: dId } });
      await prisma.room.deleteMany({ where: { dormitoryId: dId } });
  ...
  // --------------------------------------------------------------------------
  // TEST 32: Selected-Cycle Daily Tail Isolation
  // --------------------------------------------------------------------------
  it('32. Daily Tail Isolation: August unpaid Daily stay does NOT cause isDailyUnpaid in September', async () => {
    const dId = crypto.randomUUID();
    await prisma.dormitory.create({
      data: {
        id: dId,
        name: 'Dorm Tail Isolation Test',
        billingSettings: {
          create: {
            waterBillingType: 'per_unit',
            waterRate: toDecimal('18.00'),
    ...
    const tailRoom = await prisma.room.create({
      data: {
        dormitoryId: dId,
        buildingId: bld.id,
        roomNumber: 'B-TAIL-1',
        normalizedRoomNumber: 'B-TAIL-1',
        roomType: 'standard',
        monthlyRent: 3000,
        status: 'vacant',
      },
    ...
    try {
      const stayAug = await prisma.dailyStay.create({
        data: {
          dormitoryId: dId,
          roomId: tailRoom.id,
          applicantFullName: 'สายัณห์ ค้างจ่ายสิงหา',
          startDate: new Date('2026-08-05T00:00:00.000Z'),
          endDate: new Date('2026-08-07T00:00:00.000Z'),
          checkInAt: new Date('2026-08-05T07:00:00.000Z'),
          checkOutAt: new Date('2026-08-07T05:00:00.000Z'),
          inclusiveDayCount: 2,
          dailyRateAmount: 500,
          totalRentAmount: 1000,
          depositAmount: 0,
      ...
      const inv = await prisma.dailyStayInvoice.create({
        data: {
          dormitoryId: dId,
          dailyStayId: stayAug.id,
          invoiceNumber: `INV-D-2026-08-9992`,
          totalAgreedAmount: toDecimal('1000.00'),
          totalRentAmount: toDecimal('1000.00'),
          depositAmount: toDecimal('0.00'),
          status: 'ISSUED',
        },
      });
      ...
      // August preview: isDailyUnpaid = true, tenantName = 'สายัณห์ ค้างจ่ายสิงหา', hasBookableGap = true
      const augPreview = await meterService.getMeterBillingPreviewContext(dId, augCycle.id);
      const augRoom = augPreview.rooms.find(r => r.roomId === tailRoom.id);
      expect(augRoom?.isDailyUnpaid).toBe(true);
      expect(augRoom?.tenantName).toBe('สายัณห์ ค้างจ่ายสิงหา');
      expect(augRoom?.hasBookableGap).toBe(true);

      // September preview: isDailyUnpaid must be FALSE and tenantName must be null (no leakage)
      const sepPreview = await meterService.getMeterBillingPreviewContext(dId, sepCycle.id);
      const sepRoom = sepPreview.rooms.find(r => r.roomId === tailRoom.id);
      expect(sepRoom?.isDailyUnpaid).toBe(false);
      expect(sepRoom?.tenantName).toBeNull();
      expect(sepRoom?.hasBookableGap).toBe(true);

      await prisma.dailyStayInvoiceItem.deleteMany({ where: { invoiceId: inv.id } });
      await prisma.dailyStayInvoice.deleteMany({ where: { id: inv.id } });
      await prisma.dailyStay.deleteMany({ where: { id: stayAug.id } });
    } finally {
      await prisma.billingRateSnapshot.deleteMany({ where: { dormitoryId: dId } });
      await prisma.billingCycle.deleteMany({ where: { dormitoryId: dId } });
      await prisma.room.deleteMany({ where: { dormitoryId: dId } });
  ...
  // --------------------------------------------------------------------------
  // TEST 33: hasBookableGap Physical Interval Matrix
  // --------------------------------------------------------------------------
  it('33. hasBookableGap Matrix: Full-cycle contract blocks gap (false), partial contract leaves gap (true)', async () => {
    const dId = crypto.randomUUID();
    await prisma.dormitory.create({
      data: {
        id: dId,
        name: 'Dorm Gap Matrix Test',
        billingSettings: {
          create: {
            waterBillingType: 'per_unit',
            waterRate: toDecimal('18.00'),
    ...
    const fullRoom = await prisma.room.create({
      data: {
        dormitoryId: dId,
        buildingId: bld.id,
        roomNumber: 'B-GAP-FULL',
        normalizedRoomNumber: 'B-GAP-FULL',
        roomType: 'standard',
        monthlyRent: 4000,
        status: 'vacant',
      },
    ...
    const partRoom = await prisma.room.create({
      data: {
        dormitoryId: dId,
        buildingId: bld.id,
        roomNumber: 'B-GAP-PART',
        normalizedRoomNumber: 'B-GAP-PART',
        roomType: 'standard',
        monthlyRent: 4000,
        status: 'vacant',
      },
    ...
    const testTenant = await prisma.tenant.create({
      data: {
        dormitoryId: dId,
        tenantNumber: `TN-${crypto.randomUUID().slice(0, 8)}`,
        firstName: 'ผู้เช่า',
        lastName: 'ทดสอบแก๊ป',
        displayName: 'ผู้เช่าทดสอบแก๊ป',
        phone: '0819999999',
        status: 'active',
      },
    });
    ...
    try {
      // Full-cycle contract covering all August (Aug 1 - Aug 31)
      const fullContract = await prisma.contract.create({
        data: {
          dormitoryId: dId,
          roomId: fullRoom.id,
          tenantId: testTenant.id,
          contractNumber: 'CTR-FULL-01',
          rentAmount: 4000,
          depositAmount: 4000,
      ...
      // Partial contract ending August 15
      const partContract = await prisma.contract.create({
        data: {
          dormitoryId: dId,
          roomId: partRoom.id,
          tenantId: testTenant.id,
          contractNumber: 'CTR-PART-01',
          rentAmount: 4000,
          depositAmount: 4000,
          startDate: new Date('2026-08-01T00:00:00.000Z'),
      ...
      const preview = await meterService.getMeterBillingPreviewContext(dId, augCycle.id);
      const resFull = preview.rooms.find(r => r.roomId === fullRoom.id);
      const resPart = preview.rooms.find(r => r.roomId === partRoom.id);

      // Full-cycle: hasBookableGap is FALSE
      expect(resFull?.hasBookableGap).toBe(false);

      // Partial: hasBookableGap is TRUE
      expect(resPart?.hasBookableGap).toBe(true);
    } finally {
      await prisma.contract.deleteMany({ where: { dormitoryId: dId } });
  ...
  // --------------------------------------------------------------------------
  // TEST 34: Explicit Invalid Previous & Current Meter Fail-Closed Safety
  // --------------------------------------------------------------------------
  it('34. Invalid Meter Input Safety: Explicitly invalid string in elecPrev/waterPrev or elecCurr/waterCurr fails closed with 400', async () => {
    const dId = crypto.randomUUID();
    await prisma.dormitory.create({
      data: {
        id: dId,
        name: 'Dorm Invalid Safety Test',
        billingSettings: {
          create: {
            waterBillingType: 'per_unit',
            waterRate: toDecimal('18.00'),
    ...
    const safetyRoom = await prisma.room.create({
      data: {
        dormitoryId: dId,
        buildingId: bld.id,
        roomNumber: 'B-SAFE-1',
        normalizedRoomNumber: 'B-SAFE-1',
        roomType: 'standard',
        monthlyRent: 3000,
        status: 'vacant',
      },
    ...
    try {
      // 1. Invalid elecPrev ('abc')
      await expect(
        meterService.saveBulkMeterWorkspace(
          dId,
          {
            billingCycleId: augCycle.id,
            rows: [{ roomId: safetyRoom.id, elecPrev: 'abc', elecCurr: '100' }],
          },
          'user-owner-1'
        )
      ).rejects.toThrow();

      // 2. Negative waterPrev ('-5')
      await expect(
        meterService.saveBulkMeterWorkspace(
          dId,
          {
            billingCycleId: augCycle.id,
            rows: [{ roomId: safetyRoom.id, waterPrev: '-5', waterCurr: '10' }],
          },
          'user-owner-1'
        )
      ).rejects.toThrow();

      // 3. Invalid elecCurr ('xyz')
      await expect(
        meterService.saveBulkMeterWorkspace(
          dId,
          {
            billingCycleId: augCycle.id,
            rows: [{ roomId: safetyRoom.id, elecPrev: '50', elecCurr: 'xyz' }],
          },
          'user-owner-1'
        )
      ).rejects.toThrow();
  ...
  // --------------------------------------------------------------------------
  // TEST 35: Monthly Contract: Date-Only Inclusive End Date -> Exclusive +1 Day
  // --------------------------------------------------------------------------
  it('35. Monthly Contract: Inclusive end-date converts to exclusive +1 day Bangkok boundary', async () => {
    const dId = crypto.randomUUID();
    await prisma.dormitory.create({
      data: {
        id: dId,
        name: 'Dorm Inclusive End Test',
        billingSettings: {
          create: {
            waterBillingType: 'per_unit',
            waterRate: toDecimal('18.00'),
    ...
    const defaultRateSnapshot = (dormId: string) => ({
      create: {
        dormitoryId: dormId,
        waterBillingType: 'per_unit',
        electricityBillingType: 'per_unit',
    ...
    // September 2026 cycle: 2026-09-01 00:00 to 2026-09-30 23:59:59 (exclusive upper bound = 2026-10-01 00:00 Bangkok)
    const sepCycle = await prisma.billingCycle.create({
      data: {
        dormitoryId: dId,
        cycleCode: '2026-09',
        ...
        billingDate: new Date('2026-09-01T00:00:00.000Z'),
        status: 'draft',
        rateSnapshot: defaultRateSnapshot(dId),
      },
    });

    const rFull = await prisma.room.create({
      data: {
        dormitoryId: dId,
        buildingId: bld.id,
        roomNumber: 'R-FULL-SEP',
        normalizedRoomNumber: 'R-FULL-SEP',
        roomType: 'standard',
        monthlyRent: 4000,
        status: 'occupied',
      },
    ...
    const rPart = await prisma.room.create({
      data: {
        dormitoryId: dId,
        buildingId: bld.id,
        roomNumber: 'R-PART-SEP',
        normalizedRoomNumber: 'R-PART-SEP',
        roomType: 'standard',
        monthlyRent: 4000,
        status: 'occupied',
      },
    ...
    const tenant = await prisma.tenant.create({
      data: {
        ...
        tenantNumber: `TN-${crypto.randomUUID().slice(0, 8)}`,
        firstName: 'ทดสอบ',
        lastName: 'สัญญารายเดือน',
        displayName: 'ทดสอบ สัญญารายเดือน',
        status: 'active',
      },
    });

    try {
      // Full September contract: 2026-09-01 to 2026-09-30 inclusive -> [2026-09-01 00:00, 2026-10-01 00:00 Bangkok)
      // Completely covers September -> hasBookableGap = false
      await prisma.contract.create({
        data: {
          dormitoryId: dId,
          roomId: rFull.id,
          tenantId: tenant.id,
          contractNumber: 'CTR-FULL-SEP',
          rentAmount: 4000,
          depositAmount: 4000,
      ...
      // Partial September contract: 2026-09-01 to 2026-09-14 inclusive -> [2026-09-01 00:00, 2026-09-15 00:00 Bangkok)
      // Sep 14 is occupied for the whole day, but Sep 15-30 is free -> hasBookableGap = true
      await prisma.contract.create({
        data: {
          dormitoryId: dId,
          roomId: rPart.id,
          tenantId: tenant.id,
          contractNumber: 'CTR-PART-SEP',
          rentAmount: 4000,
          depositAmount: 4000,
          startDate: new Date('2026-09-01T00:00:00.000Z'),
      ...
      const preview = await meterService.getMeterBillingPreviewContext(dId, sepCycle.id);
      const resFull = preview.rooms.find(r => r.roomId === rFull.id);
      const resPart = preview.rooms.find(r => r.roomId === rPart.id);
  ...
  // --------------------------------------------------------------------------
  // TEST 36: Provisional Rental Term: Date-Only Inclusive End Date -> Exclusive +1 Day
  // --------------------------------------------------------------------------
  it('36. Provisional Rental Term: Inclusive end-date converts to exclusive +1 day Bangkok boundary', async () => {
    const dId = crypto.randomUUID();
    await prisma.dormitory.create({
      data: {
        id: dId,
        name: 'Dorm Term End Test',
        billingSettings: {
          create: {
            waterBillingType: 'per_unit',
            waterRate: toDecimal('18.00'),
    ...
    const rFull = await prisma.room.create({
      data: {
        dormitoryId: dId,
        buildingId: bld.id,
        roomNumber: 'R-TERM-FULL',
        normalizedRoomNumber: 'R-TERM-FULL',
        roomType: 'standard',
        monthlyRent: 4000,
        status: 'occupied',
      },
    ...
    const rPart = await prisma.room.create({
      data: {
        dormitoryId: dId,
        buildingId: bld.id,
        roomNumber: 'R-TERM-PART',
        normalizedRoomNumber: 'R-TERM-PART',
        roomType: 'standard',
        monthlyRent: 4000,
        status: 'occupied',
      },
    ...
    const tenant = await prisma.tenant.create({
      data: {
        ...
        tenantNumber: `TN-${crypto.randomUUID().slice(0, 8)}`,
        firstName: 'ทดสอบ',
        lastName: 'ข้อตกลงชั่วคราว',
        displayName: 'ทดสอบ ข้อตกลงชั่วคราว',
        status: 'active',
      },
    });

    try {
      // Full September Term: 2026-09-01 to 2026-09-30 inclusive -> hasBookableGap = false
      await prisma.provisionalRentalTerm.create({
        data: {
          ...
          durationMonths: 1,
          unitRentAmount: 4000,
          totalRentAmount: 4000,
          status: 'ACTIVE',
        },
      });

      // Partial September Term: 2026-09-01 to 2026-09-14 inclusive -> hasBookableGap = true
      await prisma.provisionalRentalTerm.create({
        data: {
          dormitoryId: dId,
          roomId: rPart.id,
      ...
      await prisma.provisionalRentalTerm.deleteMany({ where: { dormitoryId: dId } });
      await prisma.tenant.deleteMany({ where: { id: tenant.id } });
      await prisma.billingRateSnapshot.deleteMany({ where: { dormitoryId: dId } });
      await prisma.billingCycle.deleteMany({ where: { dormitoryId: dId } });
      await prisma.room.deleteMany({ where: { dormitoryId: dId } });
  ...
  // --------------------------------------------------------------------------
  // TEST 37: Daily Stay Sub-day Overlap into Future Reservation Rejected
  // --------------------------------------------------------------------------
  it('37. Daily Stay: Sub-day overlap into subsequent contract/reservation is rejected', async () => {
    const dId = crypto.randomUUID();
    await prisma.dormitory.create({
      data: {
        id: dId,
        name: 'Dorm Subday Test',
        billingSettings: {
          create: {
            waterBillingType: 'per_unit',
            waterRate: toDecimal('18.00'),
    ...
    const room = await prisma.room.create({
      data: {
        dormitoryId: dId,
        buildingId: bld.id,
        roomNumber: 'R-SUBDAY-1',
        normalizedRoomNumber: 'R-SUBDAY-1',
        roomType: 'standard',
        monthlyRent: 4000,
        status: 'vacant',
      },
    ...
    const tenant = await prisma.tenant.create({
      data: {
        dormitoryId: dId,
        tenantNumber: `TN-${crypto.randomUUID().slice(0, 8)}`,
        firstName: 'สัญญาล่วงหน้า',
        lastName: 'ตุลาคม',
        displayName: 'สัญญาล่วงหน้า ตุลาคม',
        status: 'active',
      },
    });

    try {
      // Contract A starts Oct 01 00:00:00 Bangkok
      await prisma.contract.create({
        data: {
          dormitoryId: dId,
          roomId: room.id,
          tenantId: tenant.id,
          contractNumber: 'CTR-OCT-01',
          rentAmount: 4000,
          depositAmount: 4000,
      ...
      // Daily Stay B requests Sep 30 18:00 to Oct 01 12:00 (overlaps Oct 01 00:00 to 12:00)
      const overlapCheck = await dailyStayService.checkRoomAvailability(
        dId,
        room.id,
        new Date('2026-09-30T11:00:00.000Z'), // Sep 30 18:00 Bangkok
        new Date('2026-10-01T05:00:00.000Z')  // Oct 01 12:00 Bangkok
      );

      expect(overlapCheck.available).toBe(false);
      expect(overlapCheck.reason).toBe('ROOM_OCCUPIED_BY_CONTRACT');
    } finally {
      await prisma.contract.deleteMany({ where: { dormitoryId: dId } });
      await prisma.tenant.deleteMany({ where: { id: tenant.id } });
      await prisma.room.deleteMany({ where: { dormitoryId: dId } });
  ...
  // --------------------------------------------------------------------------
  // TEST 38: Daily Stay Exact Boundary Non-Overlap Allowed
  // --------------------------------------------------------------------------
  it('38. Daily Stay: Exact boundary non-overlap ([Sep 29 14:00, Oct 01 00:00) vs Oct 01 00:00) is available', async () => {
    const dId = crypto.randomUUID();
    await prisma.dormitory.create({
      data: {
        id: dId,
    ...
    const room = await prisma.room.create({
      data: {
        dormitoryId: dId,
        buildingId: bld.id,
        roomNumber: 'R-BOUND-1',
        normalizedRoomNumber: 'R-BOUND-1',
        roomType: 'standard',
        monthlyRent: 4000,
        status: 'vacant',
      },
    ...
    try {
      ...
      // Daily Stay B requests Sep 29 14:00 to Oct 01 00:00 Bangkok
      const boundaryCheck = await dailyStayService.checkRoomAvailability(
        dId,
        room.id,
        new Date('2026-09-29T07:00:00.000Z'), // Sep 29 14:00 Bangkok
        new Date('2026-09-30T17:00:00.000Z')  // Oct 01 00:00 Bangkok
      );

      // In half-open interval [Sep 29 14:00, Oct 01 00:00) vs [Oct 01 00:00, ...), no overlap!
      expect(boundaryCheck.available).toBe(true);
    } finally {
      await prisma.contract.deleteMany({ where: { dormitoryId: dId } });
      await prisma.tenant.deleteMany({ where: { id: tenant.id } });
      await prisma.room.deleteMany({ where: { dormitoryId: dId } });
  ...
  // --------------------------------------------------------------------------
  // TEST 39: hasBookableGap Uses Exact Daily Timestamps
  // --------------------------------------------------------------------------
  it('39. Daily Stay: hasBookableGap uses exact Daily timestamps without gaps', async () => {
    const dId = crypto.randomUUID();
    await prisma.dormitory.create({
      data: {
        id: dId,
        name: 'Dorm Daily Gap Test',
        billingSettings: {
          create: {
            waterBillingType: 'per_unit',
            waterRate: toDecimal('18.00'),
    ...
    const room = await prisma.room.create({
      data: {
        dormitoryId: dId,
        buildingId: bld.id,
        roomNumber: 'R-DAILY-CONT',
        normalizedRoomNumber: 'R-DAILY-CONT',
        roomType: 'standard',
        monthlyRent: 4000,
        status: 'occupied',
      },
    ...
    const tenant = await prisma.tenant.create({
      data: {
        ...
        tenantNumber: `TN-${crypto.randomUUID().slice(0, 8)}`,
        firstName: 'สมชาย',
        lastName: 'รายวันเต็มเดือน',
        displayName: 'สมชาย รายวันเต็มเดือน',
        status: 'active',
      },
    });

    try {
      // Stay 1: Sep 1 00:00 Bangkok to Sep 15 12:00 Bangkok
      await prisma.dailyStay.create({
        data: {
          ...
          tenantId: tenant.id,
          requestSource: 'OWNER',
          applicantFullName: 'สมชาย รายวันเต็มเดือน',
          startDate: new Date('2026-09-01T00:00:00.000Z'),
          endDate: new Date('2026-09-15T00:00:00.000Z'),
          checkInAt: new Date('2026-08-31T17:00:00.000Z'), // Sep 1 00:00 Bangkok
          checkOutAt: new Date('2026-09-15T05:00:00.000Z'), // Sep 15 12:00 Bangkok
          inclusiveDayCount: 15,
          dailyRateAmount: 500,
          totalRentAmount: 7500,
          depositAmount: 0,
          status: 'ACTIVE',
        },
      });

      // Stay 2: Sep 15 12:00 Bangkok to Oct 1 00:00 Bangkok
      await prisma.dailyStay.create({
        data: {
          dormitoryId: dId,
          roomId: room.id,
          ...
          endDate: new Date('2026-09-30T00:00:00.000Z'),
          checkInAt: new Date('2026-09-15T05:00:00.000Z'), // Sep 15 12:00 Bangkok
          checkOutAt: new Date('2026-09-30T17:00:00.000Z'), // Oct 1 00:00 Bangkok
          inclusiveDayCount: 16,
          dailyRateAmount: 500,
          totalRentAmount: 8000,
          depositAmount: 0,
          status: 'ACTIVE',
        },
      });
      ...
      const preview = await meterService.getMeterBillingPreviewContext(dId, sepCycle.id);
      const res = preview.rooms.find(r => r.roomId === room.id);

      // Continuous coverage across September -> hasBookableGap = false
      expect(res?.hasBookableGap).toBe(false);
    } finally {
      await prisma.dailyStay.deleteMany({ where: { dormitoryId: dId } });
      await prisma.tenant.deleteMany({ where: { id: tenant.id } });
      await prisma.billingRateSnapshot.deleteMany({ where: { dormitoryId: dId } });
      await prisma.billingCycle.deleteMany({ where: { dormitoryId: dId } });
      await prisma.room.deleteMany({ where: { dormitoryId: dId } });
  ...
  // --------------------------------------------------------------------------
  // TEST 40: Canonical Payment Flow Clears Daily Red Tail on Meter Preview
  // --------------------------------------------------------------------------
  it('40. Canonical Payment: Settle DAILY_RENT clears unpaid red tail on Meter preview', async () => {
    const dId = crypto.randomUUID();
    await prisma.dormitory.create({
      data: {
        id: dId,
        name: 'Dorm Payment Flow Test',
        billingSettings: {
          create: {
            waterBillingType: 'per_unit',
            waterRate: toDecimal('18.00'),
    ...
    const room = await prisma.room.create({
      data: {
        dormitoryId: dId,
        buildingId: bld.id,
        roomNumber: 'R-PAY-TAIL',
        normalizedRoomNumber: 'R-PAY-TAIL',
        roomType: 'standard',
        monthlyRent: 4000,
        status: 'vacant',
      },
    ...
    const tenant = await prisma.tenant.create({
      data: {
        ...
        lastName: 'ชำระค่าเช่า',
        displayName: 'ก้องเกียรติ ชำระค่าเช่า',
        status: 'active',
      },
    });
    ...
    try {
      const stay = await prisma.dailyStay.create({
        data: {
          ...
          tenantId: tenant.id,
          requestSource: 'OWNER',
          applicantFullName: 'ก้องเกียรติ ชำระค่าเช่า',
          startDate: new Date('2026-08-05T00:00:00.000Z'),
          endDate: new Date('2026-08-07T00:00:00.000Z'),
          checkInAt: new Date('2026-08-05T07:00:00.000Z'),
          checkOutAt: new Date('2026-08-07T05:00:00.000Z'),
      ...
      const inv = await prisma.dailyStayInvoice.create({
        data: {
          dormitoryId: dId,
          dailyStayId: stay.id,
          invoiceNumber: `INV-D-PAY-01`,
          totalAgreedAmount: toDecimal('1700.00'),
          totalRentAmount: toDecimal('1200.00'),
          depositAmount: toDecimal('500.00'),
          outstandingAmount: toDecimal('1200.00'),
          status: 'ISSUED',
        },
      });
      ...
      await prisma.dailyStayInvoiceItem.create({
        data: {
          invoiceId: inv.id,
          itemType: 'DEPOSIT',
          description: 'เงินประกันห้องพัก',
          amount: 500,
          status: 'DECLARED_PAID',
          paidAt: new Date('2026-08-05T07:00:00.000Z'),
        },
      });

      // Starting state: DAILY_RENT = OUTSTANDING -> preview shows isDailyUnpaid = true
      const previewBefore = await meterService.getMeterBillingPreviewContext(dId, augCycle.id);
      const roomBefore = previewBefore.rooms.find(r => r.roomId === room.id);
      expect(roomBefore?.isDailyUnpaid).toBe(true);
      expect(roomBefore?.tenantName).toBe('ก้องเกียรติ ชำระค่าเช่า');

      // Canonical payment action
      const settleResult = await dailyStayService.settleDailyStayInvoiceItem(
        dId,
        inv.id,
        'DAILY_RENT',
        'user-owner-test'
      ...
      // Verify DB: DAILY_RENT is SETTLED, invoice status is PAID, paidAt is set
      expect(settleResult.status).toBe('PAID');
      const rentItem = settleResult.items.find((i: any) => i.itemType === 'DAILY_RENT');
      expect(rentItem?.status).toBe('SETTLED');
      expect(rentItem?.paidAt).toBeInstanceOf(Date);

      // Verify Meter preview: isDailyUnpaid = false
      const previewAfter = await meterService.getMeterBillingPreviewContext(dId, augCycle.id);
      const roomAfter = previewAfter.rooms.find(r => r.roomId === room.id);
      expect(roomAfter?.isDailyUnpaid).toBe(false);
    } finally {
      await prisma.dailyStayInvoiceItem.deleteMany({ where: { invoice: { dormitoryId: dId } } });
      await prisma.dailyStayInvoice.deleteMany({ where: { dormitoryId: dId } });
      await prisma.dailyStay.deleteMany({ where: { dormitoryId: dId } });
      await prisma.tenant.deleteMany({ where: { id: tenant.id } });
      await prisma.billingRateSnapshot.deleteMany({ where: { dormitoryId: dId } });
      await prisma.billingCycle.deleteMany({ where: { dormitoryId: dId } });
  ...
  // --------------------------------------------------------------------------
  // TEST 41: paidAt First-Write Immutability
  // --------------------------------------------------------------------------
  it('41. paidAt Immutability: First payment sets paidAt; repeated operation preserves original timestamp', async () => {
    const dId = crypto.randomUUID();
    await prisma.dormitory.create({
      data: {
        id: dId,
        name: 'Dorm Immutability Test',
        billingSettings: {
          create: {
            waterBillingType: 'per_unit',
            waterRate: toDecimal('18.00'),
    ...
    const room = await prisma.room.create({
      data: {
        dormitoryId: dId,
        buildingId: bld.id,
        roomNumber: 'R-IMMUT-1',
        normalizedRoomNumber: 'R-IMMUT-1',
        roomType: 'standard',
        monthlyRent: 4000,
        status: 'vacant',
      },
    ...
    try {
      const originalTime = new Date('2026-08-01T10:00:00.000Z');
      ...
      const stay = await prisma.dailyStay.create({
        data: {
          ...
          roomId: room.id,
          requestSource: 'OWNER',
          applicantFullName: 'นรินทร์ ไม่เขียนทับเวลา',
          startDate: new Date('2026-08-01T00:00:00.000Z'),
      ...
      const inv = await prisma.dailyStayInvoice.create({
        data: {
          dormitoryId: dId,
          dailyStayId: stay.id,
          invoiceNumber: `INV-IMMUT-01`,
          totalAgreedAmount: toDecimal('500.00'),
          totalRentAmount: toDecimal('500.00'),
          status: 'ISSUED',
        },
      });
      ...
      await prisma.dailyStayInvoiceItem.create({
        data: {
          invoiceId: inv.id,
          itemType: 'DAILY_RENT',
          description: 'ค่าเช่ารายวัน',
          amount: 500,
          status: 'OUTSTANDING',
          paidAt: originalTime, // Already recorded timestamp
        },
      });
      ...
      const rentItem = result.items.find((i: any) => i.itemType === 'DAILY_RENT');
      expect(rentItem?.paidAt?.toISOString()).toBe(originalTime.toISOString());
    } finally {
      await prisma.dailyStayInvoiceItem.deleteMany({ where: { invoice: { dormitoryId: dId } } });
      await prisma.dailyStayInvoice.deleteMany({ where: { dormitoryId: dId } });
      await prisma.dailyStay.deleteMany({ where: { dormitoryId: dId } });
  ...
  // --------------------------------------------------------------------------
  // TEST 42: Legacy Financially Paid Record with paidAt: null Recognized as Paid
  // --------------------------------------------------------------------------
  it('42. Legacy Paid Record: Financially SETTLED with paidAt: null is recognized as paid without fabricating timestamp', async () => {
    const dId = crypto.randomUUID();
    await prisma.dormitory.create({
      data: {
        id: dId,
        name: 'Dorm Legacy Paid Test',
        billingSettings: {
          create: {
            waterBillingType: 'per_unit',
            waterRate: toDecimal('18.00'),
    ...
    const room = await prisma.room.create({
      data: {
        dormitoryId: dId,
        buildingId: bld.id,
        roomNumber: 'R-LEGACY-PAID',
        normalizedRoomNumber: 'R-LEGACY-PAID',
        roomType: 'standard',
        monthlyRent: 4000,
        status: 'vacant',
      },
    ...
    try {
      const stay = await prisma.dailyStay.create({
        data: {
          ...
          roomId: room.id,
          requestSource: 'OWNER',
          applicantFullName: 'ประดิษฐ์ ระบบเก่า',
          startDate: new Date('2026-08-01T00:00:00.000Z'),
          endDate: new Date('2026-08-02T00:00:00.000Z'),
          inclusiveDayCount: 1,
          status: 'CHECKED_OUT',
      ...
      // Must be recognized as paid (isDailyUnpaid = false)
      expect(res?.isDailyUnpaid).toBe(false);

      // Verify DB paidAt remained null (did not fabricate timestamp)
      const dbItem = await prisma.dailyStayInvoiceItem.findFirst({ where: { invoiceId: inv.id } });
      expect(dbItem?.paidAt).toBeNull();
    } finally {
      await prisma.dailyStayInvoiceItem.deleteMany({ where: { invoice: { dormitoryId: dId } } });
      await prisma.dailyStayInvoice.deleteMany({ where: { dormitoryId: dId } });
      await prisma.dailyStay.deleteMany({ where: { dormitoryId: dId } });
  ...
  // --------------------------------------------------------------------------
  // TEST 43: Deposit / Rent Independence
  // --------------------------------------------------------------------------
  it('43. Deposit / Rent Independence: Deposit status does not decide red Daily Rent badge', async () => {
    const dId = crypto.randomUUID();
    await prisma.dormitory.create({
      data: {
        id: dId,
        name: 'Dorm Independence Test',
        billingSettings: {
          create: {
            waterBillingType: 'per_unit',
            waterRate: toDecimal('18.00'),
    ...
    const rDepPaidRentUnpaid = await prisma.room.create({
      data: {
        dormitoryId: dId,
        buildingId: bld.id,
        roomNumber: 'R-INDEP-1',
        normalizedRoomNumber: 'R-INDEP-1',
        roomType: 'standard',
        monthlyRent: 4000,
        status: 'vacant',
      },
    ...
    const rRentPaidDepUnpaid = await prisma.room.create({
      data: {
        dormitoryId: dId,
        buildingId: bld.id,
        roomNumber: 'R-INDEP-2',
        normalizedRoomNumber: 'R-INDEP-2',
        roomType: 'standard',
        monthlyRent: 4000,
        status: 'vacant',
      },
    ...
    try {
      // Case A: Deposit = DECLARED_PAID, Rent = OUTSTANDING -> isDailyUnpaid = true
      const stayA = await prisma.dailyStay.create({
        data: {
          dormitoryId: dId,
          roomId: rDepPaidRentUnpaid.id,
          requestSource: 'OWNER',
          applicantFullName: 'เคสเอ มัดจำจ่ายแล้วค่าเช่าค้าง',
          startDate: new Date('2026-08-01T00:00:00.000Z'),
          endDate: new Date('2026-08-02T00:00:00.000Z'),
          inclusiveDayCount: 1,
          status: 'CHECKED_OUT',
      ...
      const invA = await prisma.dailyStayInvoice.create({
        data: {
          dormitoryId: dId,
          dailyStayId: stayA.id,
          invoiceNumber: `INV-INDEP-A`,
          totalAgreedAmount: toDecimal('1000.00'),
          totalRentAmount: toDecimal('500.00'),
          depositAmount: toDecimal('500.00'),
          status: 'ISSUED',
      ...
      // Case B: Rent = SETTLED, Deposit = OUTSTANDING -> isDailyUnpaid = false
      const stayB = await prisma.dailyStay.create({
        data: {
          dormitoryId: dId,
          roomId: rRentPaidDepUnpaid.id,
          requestSource: 'OWNER',
          applicantFullName: 'เคสบี ค่าเช่าจ่ายแล้วมัดจำค้าง',
          startDate: new Date('2026-08-01T00:00:00.000Z'),
          endDate: new Date('2026-08-02T00:00:00.000Z'),
          inclusiveDayCount: 1,
          status: 'CHECKED_OUT',
      ...
      const invB = await prisma.dailyStayInvoice.create({
        data: {
          dormitoryId: dId,
          dailyStayId: stayB.id,
          invoiceNumber: `INV-INDEP-B`,
          totalAgreedAmount: toDecimal('1000.00'),
          totalRentAmount: toDecimal('500.00'),
          depositAmount: toDecimal('500.00'),
          status: 'ISSUED',
      ...
      const preview = await meterService.getMeterBillingPreviewContext(dId, augCycle.id);
      const resA = preview.rooms.find(r => r.roomId === rDepPaidRentUnpaid.id);
      const resB = preview.rooms.find(r => r.roomId === rRentPaidDepUnpaid.id);

      // Proof:
      expect(resA?.isDailyUnpaid).toBe(true);  // Deposit paid, Rent unpaid -> unpaid red tail
      expect(resB?.isDailyUnpaid).toBe(false); // Rent paid, Deposit unpaid -> no red tail
    } finally {
      await prisma.dailyStayInvoiceItem.deleteMany({ where: { invoice: { dormitoryId: dId } } });
      await prisma.dailyStayInvoice.deleteMany({ where: { dormitoryId: dId } });
      await prisma.dailyStay.deleteMany({ where: { dormitoryId: dId } });
  ...
  // --------------------------------------------------------------------------
  // TEST 44: Realistic Room 104 Zero-Payable Fixture with Complete Snapshot Readings
  // --------------------------------------------------------------------------
  it('44. Realistic Zero-Payable Fixture: Populated meter snapshot values yield 0.00 ฿ utility contribution', async () => {
    const dId = crypto.randomUUID();
    await prisma.dormitory.create({
      data: {
        id: dId,
        name: 'Dorm Zero Payable Test',
        billingSettings: {
          create: {
            waterBillingType: 'per_unit',
            waterRate: toDecimal('18.00'),
    ...
    try {
      // Save workspace row for Room 104 with complete snapshot readings (water 35 -> 35, elec 120 -> 120)
      const saveRes = await meterService.saveBulkMeterWorkspace(
        dId,
        {
          billingCycleId: augCycle.id,
      ...
      // Verify meter readings in DB are fully populated
      const waterReading = await prisma.meterReading.findFirst({
        where: { roomId: room104.id, meterDeviceId: waterDev.id, billingCycleId: augCycle.id },
      });
      const elecReading = await prisma.meterReading.findFirst({
        where: { roomId: room104.id, meterDeviceId: elecDev.id, billingCycleId: augCycle.id },
      });

      expect(Number(waterReading?.previousReading)).toBe(35);
      expect(Number(waterReading?.currentReading)).toBe(35);
      expect(Number(waterReading?.usageUnits)).toBe(0);

      expect(Number(elecReading?.previousReading)).toBe(120);
      expect(Number(elecReading?.currentReading)).toBe(120);
      expect(Number(elecReading?.usageUnits)).toBe(0);

      // Verify preview calculation fields for Room 104
      const preview = await meterService.getMeterBillingPreviewContext(dId, augCycle.id);
      const r104Preview = preview.rooms.find(r => r.roomId === room104.id);

      expect(r104Preview?.rentAmount).toBe('0.00');
      expect(r104Preview?.snapshotOtherFees).toEqual([]);
    } finally {
      await prisma.meterReading.deleteMany({ where: { dormitoryId: dId } });
      await prisma.meterDevice.deleteMany({ where: { dormitoryId: dId } });
      await prisma.billingRateSnapshot.deleteMany({ where: { dormitoryId: dId } });
      await prisma.billingCycle.deleteMany({ where: { dormitoryId: dId } });
      await prisma.room.deleteMany({ where: { dormitoryId: dId } });
      await prisma.building.deleteMany({ where: { dormitoryId: dId } });
  ...
  // --------------------------------------------------------------------------
  // TEST 51: Rent 2500 OUTSTANDING / Deposit 500 SETTLED -> isDailyUnpaid = true
  // --------------------------------------------------------------------------
  it('51. Deposit settled (500) but Rent outstanding (2500) leaves isDailyUnpaid = true', async () => {
    const dId = crypto.randomUUID();
    const bldId = crypto.randomUUID();
    const roomId = crypto.randomUUID();

    await prisma.dormitory.create({ data: { id: dId, name: 'Dorm Matrix A Test' } });
    await prisma.building.create({ data: { id: bldId, dormitoryId: dId, name: 'Bld A' } });
    await prisma.room.create({
      data: {
        id: roomId,
    ...
    const stay = await prisma.dailyStay.create({
      data: {
        dormitoryId: dId,
        roomId,
        applicantFullName: 'Deposit Settled Guest 51',
        startDate: new Date('2026-08-10T00:00:00.000Z'),
        endDate: new Date('2026-08-15T00:00:00.000Z'),
        checkInAt: new Date('2026-08-10T07:00:00.000Z'),
        actualCheckedOutAt: new Date('2026-08-15T03:00:00.000Z'),
        inclusiveDayCount: 5,
        status: 'CHECKED_OUT',
      },
    });
    ...
    const invoice = await prisma.dailyStayInvoice.create({
      data: {
        dormitoryId: dId,
        dailyStayId: stay.id,
        invoiceNumber: 'D-INV-51',
        totalRentAmount: toDecimal('2500.00'),
        depositAmount: toDecimal('500.00'),
        totalAgreedAmount: toDecimal('3000.00'),
        outstandingAmount: toDecimal('3000.00'),
        status: 'ISSUED',
        items: {
          create: [
            { itemType: 'DAILY_RENT', description: 'Daily Rent', amount: toDecimal('2500.00'), status: 'OUTSTANDING' },
            { itemType: 'DEPOSIT', description: 'Deposit', amount: toDecimal('500.00'), status: 'OUTSTANDING' },
          ],
        },
      },
    });
    ...
    try {
      // Settle DEPOSIT only
      const settledInvoice = await dailyStayService.settleDailyStayInvoiceItem(
        dId,
        invoice.id,
        'DEPOSIT',
        crypto.randomUUID()
      ...
      // Verify aggregate invoice balance
      expect(settledInvoice.status).toBe('PARTIALLY_PAID');
      expect(settledInvoice.outstandingAmount.toString()).toBe('2500');

      // Verify meter preview: Rent is still unpaid -> isDailyUnpaid = true
      const preview = await meterService.getMeterBillingPreviewContext(dId, cycle.id);
      const roomRow = preview.rooms.find((r) => r.roomId === roomId);
      expect(roomRow?.isDailyUnpaid).toBe(true);
    } finally {
      await prisma.dailyStayInvoiceItem.deleteMany({ where: { invoiceId: invoice.id } });
      await prisma.dailyStayInvoice.deleteMany({ where: { id: invoice.id } });
      await prisma.dailyStay.deleteMany({ where: { id: stay.id } });
      await prisma.billingCycle.deleteMany({ where: { id: cycle.id } });
      await prisma.room.deleteMany({ where: { id: roomId } });
      await prisma.building.deleteMany({ where: { id: bldId } });
      await prisma.dormitory.deleteMany({ where: { id: dId } });
    }
  ...
  // --------------------------------------------------------------------------
  // TEST 52: Rent 2500 SETTLED / Deposit 500 OUTSTANDING -> isDailyUnpaid = false
  // --------------------------------------------------------------------------
  it('52. Rent settled (2500) but Deposit outstanding (500) clears isDailyUnpaid = false', async () => {
    const dId = crypto.randomUUID();
    const bldId = crypto.randomUUID();
    const roomId = crypto.randomUUID();

    await prisma.dormitory.create({ data: { id: dId, name: 'Dorm Matrix B Test' } });
    await prisma.building.create({ data: { id: bldId, dormitoryId: dId, name: 'Bld A' } });
    await prisma.room.create({
      data: {
        id: roomId,
    ...
    const stay = await prisma.dailyStay.create({
      data: {
        dormitoryId: dId,
        roomId,
        applicantFullName: 'Rent Settled Guest 52',
        startDate: new Date('2026-08-10T00:00:00.000Z'),
        endDate: new Date('2026-08-15T00:00:00.000Z'),
        checkInAt: new Date('2026-08-10T07:00:00.000Z'),
        actualCheckedOutAt: new Date('2026-08-15T03:00:00.000Z'),
    ...
    try {
      ...
      // Verify aggregate invoice balance (still partially paid because deposit is 500 outstanding)
      expect(settledInvoice.status).toBe('PARTIALLY_PAID');
      expect(settledInvoice.outstandingAmount.toString()).toBe('500');

      // Verify meter preview: Rent is settled -> isDailyUnpaid = false (red tail disappears)
      const preview = await meterService.getMeterBillingPreviewContext(dId, cycle.id);
      const roomRow = preview.rooms.find((r) => r.roomId === roomId);
      expect(roomRow?.isDailyUnpaid).toBe(false);
    } finally {
      await prisma.dailyStayInvoiceItem.deleteMany({ where: { invoiceId: invoice.id } });
      await prisma.dailyStayInvoice.deleteMany({ where: { id: invoice.id } });
      await prisma.dailyStay.deleteMany({ where: { id: stay.id } });
  ...
  // --------------------------------------------------------------------------
  // TEST 53: Contract status 'draft' does NOT block physical room or bookable gap
  // --------------------------------------------------------------------------
  it('53. Contract status draft does NOT block physical room availability or bookable gap', async () => {
    const dId = crypto.randomUUID();
    const bldId = crypto.randomUUID();
    const roomId = crypto.randomUUID();
    const tenantId = crypto.randomUUID();

    await prisma.dormitory.create({ data: { id: dId, name: 'Dorm Draft Contract Test' } });
    await prisma.building.create({ data: { id: bldId, dormitoryId: dId, name: 'Bld A' } });
    await prisma.room.create({
      data: {
        id: roomId,
    ...
    try {
      // 1. Daily stay availability check: Draft contract does NOT block
      const availRes = await dailyStayService.checkRoomAvailability(
        dId,
        roomId,
        new Date('2026-09-10T00:00:00.000Z'),
      ...
      // 2. Meter preview: September remains bookable (hasBookableGap = true, billingSource = NONE)
      const preview = await meterService.getMeterBillingPreviewContext(dId, cycle.id);
      const roomRow = preview.rooms.find((r) => r.roomId === roomId);
      expect(roomRow?.hasBookableGap).toBe(true);
      expect(roomRow?.billingSource).toBe('NONE');
    } finally {
      await prisma.contract.deleteMany({ where: { id: draftContract.id } });
      await prisma.billingCycle.deleteMany({ where: { id: cycle.id } });
      await prisma.tenant.deleteMany({ where: { id: tenantId } });
      await prisma.room.deleteMany({ where: { id: roomId } });
      await prisma.building.deleteMany({ where: { id: bldId } });
  ...
  // --------------------------------------------------------------------------
  // TEST 54: Contract status 'approved' & 'pending_signature' DO block occupancy
  // --------------------------------------------------------------------------
  it('54. Contract status approved and pending_signature DO block physical room availability', async () => {
    const dId = crypto.randomUUID();
    const bldId = crypto.randomUUID();
    const roomId = crypto.randomUUID();
    const tenantId = crypto.randomUUID();

    await prisma.dormitory.create({ data: { id: dId, name: 'Dorm Approved/Pending Contract Test' } });
    await prisma.building.create({ data: { id: bldId, dormitoryId: dId, name: 'Bld A' } });
    await prisma.room.create({
      data: {
        id: roomId,
    ...
    const approvedContract = await prisma.contract.create({
      data: {
        dormitoryId: dId,
        contractNumber: 'CTR-APPROVED-54',
        roomId,
        tenantId,
        status: 'approved',
        startDate: new Date('2026-09-01T00:00:00.000Z'),
    ...
    try {
      // 1. Approved contract blocks
      const availApproved = await dailyStayService.checkRoomAvailability(
        dId,
        roomId,
        new Date('2026-09-10T00:00:00.000Z'),
        new Date('2026-09-15T00:00:00.000Z')
      );
      expect(availApproved.available).toBe(false);
      expect(availApproved.reason).toBe('ROOM_OCCUPIED_BY_CONTRACT');

      // 2. Pending signature contract blocks
      await prisma.contract.update({
        where: { id: approvedContract.id },
        data: { status: 'pending_signature' },
      });

      const availPending = await dailyStayService.checkRoomAvailability(
        dId,
        roomId,
        new Date('2026-09-10T00:00:00.000Z'),
        new Date('2026-09-15T00:00:00.000Z')
      );
      expect(availPending.available).toBe(false);
      expect(availPending.reason).toBe('ROOM_OCCUPIED_BY_CONTRACT');
    } finally {
      await prisma.contract.deleteMany({ where: { id: approvedContract.id } });
      await prisma.tenant.deleteMany({ where: { id: tenantId } });
      await prisma.room.deleteMany({ where: { id: roomId } });
      await prisma.building.deleteMany({ where: { id: bldId } });
      await prisma.dormitory.deleteMany({ where: { id: dId } });
  ...
  // --------------------------------------------------------------------------
  // TEST 55: Contract status 'ended' blocks inside historical interval, allows after endDate
  // --------------------------------------------------------------------------
  it('55. Contract status ended blocks inside historical interval, allows bookable gap after endDate', async () => {
    const dId = crypto.randomUUID();
    const bldId = crypto.randomUUID();
    const roomId = crypto.randomUUID();
    const tenantId = crypto.randomUUID();

    await prisma.dormitory.create({ data: { id: dId, name: 'Dorm Ended Contract Test' } });
    await prisma.building.create({ data: { id: bldId, dormitoryId: dId, name: 'Bld A' } });
    await prisma.room.create({
      data: {
        id: roomId,
    ...
    await prisma.tenant.create({
      data: {
        ...
        firstName: 'Ended',
        lastName: 'Tenant',
        displayName: 'Ended Tenant',
      },
    });

    const endedContract = await prisma.contract.create({
      data: {
        dormitoryId: dId,
        contractNumber: 'CTR-ENDED-55',
        roomId,
        tenantId,
        status: 'ended',
    ...
    try {
      // 1. Inside historical physical interval [Aug 15, Sep 14]: MUST reject overlap
      const insideAvail = await dailyStayService.checkRoomAvailability(
        dId,
        roomId,
        new Date('2026-09-05T00:00:00.000Z'),
        new Date('2026-09-10T00:00:00.000Z')
      );
      expect(insideAvail.available).toBe(false);
      expect(insideAvail.reason).toBe('ROOM_OCCUPIED_BY_CONTRACT');

      // 2. After contract endDate [Sep 15, Sep 20]: MUST be available (bookable gap)
      const afterAvail = await dailyStayService.checkRoomAvailability(
        dId,
        roomId,
        new Date('2026-09-15T00:00:00.000Z'),
      ...
      await prisma.contract.deleteMany({ where: { id: endedContract.id } });
      await prisma.tenant.deleteMany({ where: { id: tenantId } });
      await prisma.room.deleteMany({ where: { id: roomId } });
      await prisma.building.deleteMany({ where: { id: bldId } });
      await prisma.dormitory.deleteMany({ where: { id: dId } });
  ...
  // --------------------------------------------------------------------------
  // TEST 56: Contract status 'terminated' blocks up to terminationEffectiveDate, allows after
  // --------------------------------------------------------------------------
  it('56. Contract status terminated blocks up to terminationEffectiveDate, allows after', async () => {
    const dId = crypto.randomUUID();
    const bldId = crypto.randomUUID();
    const roomId = crypto.randomUUID();
    const tenantId = crypto.randomUUID();

    await prisma.dormitory.create({ data: { id: dId, name: 'Dorm Terminated Contract Test' } });
    await prisma.building.create({ data: { id: bldId, dormitoryId: dId, name: 'Bld A' } });
    await prisma.room.create({
      data: {
        id: roomId,
    ...
    await prisma.tenant.create({
      data: {
        ...
        lastName: 'Tenant',
        displayName: 'Terminated Tenant',
      },
    });

    const termContract = await prisma.contract.create({
      data: {
        dormitoryId: dId,
        contractNumber: 'CTR-TERM-56',
        roomId,
        tenantId,
        status: 'terminated',
        startDate: new Date('2026-08-15T00:00:00.000Z'),
        endDate: new Date('2026-09-30T00:00:00.000Z'),
        terminationEffectiveDate: new Date('2026-08-31T00:00:00.000Z'),
        terminatedAt: new Date('2026-08-31T12:00:00.000Z'),
        durationMonths: 2,
      },
    });

    try {
      // 1. Inside effective occupancy before termination [Aug 20, Aug 25]: MUST reject overlap
      const insideAvail = await dailyStayService.checkRoomAvailability(
        dId,
        roomId,
        new Date('2026-08-20T00:00:00.000Z'),
      ...
      // 2. After canonical termination date [Sep 5, Sep 10]: MUST be available
      const afterAvail = await dailyStayService.checkRoomAvailability(
        dId,
        roomId,
        new Date('2026-09-05T00:00:00.000Z'),
      ...
      await prisma.contract.deleteMany({ where: { id: termContract.id } });
      await prisma.tenant.deleteMany({ where: { id: tenantId } });
      await prisma.room.deleteMany({ where: { id: roomId } });
      await prisma.building.deleteMany({ where: { id: bldId } });
      await prisma.dormitory.deleteMany({ where: { id: dId } });
  ...
  // --------------------------------------------------------------------------
  // TEST 57: Settle-item aggregate calculations, totalPaidAmount & idempotency
  // --------------------------------------------------------------------------
  it('57. settleDailyStayInvoiceItem aggregate correctness, totalPaidAmount calculation and idempotency', async () => {
    const dId = crypto.randomUUID();
    const bldId = crypto.randomUUID();
    const roomId = crypto.randomUUID();

    await prisma.dormitory.create({ data: { id: dId, name: 'Dorm Settle Item Test' } });
    await prisma.building.create({ data: { id: bldId, dormitoryId: dId, name: 'Bld A' } });
    await prisma.room.create({
      data: {
        id: roomId,
    ...
    const stay = await prisma.dailyStay.create({
      data: {
        dormitoryId: dId,
        roomId,
        applicantFullName: 'Test Guest 57',
        applicantPhone: '0811111157',
        startDate: new Date('2026-07-10T00:00:00.000Z'),
        endDate: new Date('2026-07-15T00:00:00.000Z'),
        inclusiveDayCount: 5,
        dailyRateAmount: toDecimal('500.00'),
        totalRentAmount: toDecimal('2500.00'),
        depositAmount: toDecimal('500.00'),
        status: 'CHECKED_OUT',
      },
      ...
      data: {
        ...
        items: {
          create: [
            {
              itemType: 'DAILY_RENT',
              description: 'ค่าเช่ารายวัน 5 วัน',
              amount: toDecimal('2500.00'),
              status: 'OUTSTANDING',
            },
            {
              itemType: 'DEPOSIT',
    ...
    try {
      // 1. Settle Daily Rent (partial payment)
      const settleRentRes = await dailyStayService.settleDailyStayInvoiceItem(dId, invoice.id, 'DAILY_RENT');
      expect(settleRentRes.status).toBe('PARTIALLY_PAID');
      expect(Number(settleRentRes.totalAgreedAmount)).toBe(3000);
      expect(Number(settleRentRes.outstandingAmount)).toBe(500);
      expect(Number((settleRentRes as any).totalPaidAmount)).toBe(2500);

      const rentItem = settleRentRes.items.find((i: any) => i.itemType === 'DAILY_RENT');
      expect(rentItem?.status).toBe('SETTLED');
      expect(rentItem?.paidAt).toBeDefined();
      const firstPaidAt = rentItem?.paidAt;

      // 2. Replay settle Daily Rent (idempotency)
      const replayRentRes = await dailyStayService.settleDailyStayInvoiceItem(dId, invoice.id, 'DAILY_RENT');
      expect(replayRentRes.status).toBe('PARTIALLY_PAID');
      expect(Number(replayRentRes.outstandingAmount)).toBe(500);
      const replayRentItem = replayRentRes.items.find((i: any) => i.itemType === 'DAILY_RENT');
      expect(replayRentItem?.paidAt?.toISOString()).toBe(firstPaidAt?.toISOString());

      // 3. Settle Deposit (full payment)
      const settleDepositRes = await dailyStayService.settleDailyStayInvoiceItem(dId, invoice.id, 'DEPOSIT');
      expect(settleDepositRes.status).toBe('PAID');
      expect(Number(settleDepositRes.totalAgreedAmount)).toBe(3000);
      expect(Number(settleDepositRes.outstandingAmount)).toBe(0);
      expect(Number((settleDepositRes as any).totalPaidAmount)).toBe(3000);
      expect(settleDepositRes.depositDeclaredStatus).toBe('PAID');

      const depItem = settleDepositRes.items.find((i: any) => i.itemType === 'DEPOSIT');
  ...
  // --------------------------------------------------------------------------
  // TEST 58: Cross-dormitory isolation in settle-item (fail-closed)
  // --------------------------------------------------------------------------
  it('58. Cross-dormitory settleDailyStayInvoiceItem fails closed with 404', async () => {
    const dIdA = crypto.randomUUID();
    const dIdB = crypto.randomUUID();
    const bldId = crypto.randomUUID();
    const roomId = crypto.randomUUID();

    await prisma.dormitory.create({ data: { id: dIdA, name: 'Dorm A' } });
    await prisma.dormitory.create({ data: { id: dIdB, name: 'Dorm B' } });
    await prisma.building.create({ data: { id: bldId, dormitoryId: dIdA, name: 'Bld A' } });
    await prisma.room.create({
      data: {
        id: roomId,
        dormitoryId: dIdA,
    ...
    const stay = await prisma.dailyStay.create({
      data: {
        dormitoryId: dIdA,
        roomId,
        applicantFullName: 'Test Guest 58',
        applicantPhone: '0811111158',
        startDate: new Date('2026-07-10T00:00:00.000Z'),
        endDate: new Date('2026-07-15T00:00:00.000Z'),
        inclusiveDayCount: 5,
        dailyRateAmount: toDecimal('500.00'),
    ...
    try {
      // Settle attempt with mismatched Dorm B context MUST fail with 404
      await expect(
        dailyStayService.settleDailyStayInvoiceItem(dIdB, invoice.id, 'DAILY_RENT')
      ).rejects.toMatchObject({
        statusCode: 404,
      ...
      await prisma.room.deleteMany({ where: { id: roomId } });
      await prisma.building.deleteMany({ where: { id: bldId } });
      await prisma.dormitory.deleteMany({ where: { id: dIdA } });
      await prisma.dormitory.deleteMany({ where: { id: dIdB } });
    }
  });
});
